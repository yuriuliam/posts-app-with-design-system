...
type DictionaryTokenMap = Map<string, TransformedToken>
...
const createTSTokens = defineTemplate<TSTokensDictionaryData>`
...
function registerFormats() {
  ...
  StyleDictionary.registerFormat({
    ...
    format: async ({ dictionary, file }) => {
      ...
      const designTokens = omit(dictionary.tokens, typographyAndSchemeKeys)
      const tsTokens = simplifyTokens(designTokens, dictionary.tokenMap)

      const tsBody = createTSTokens({
        tokens: JSON.stringify(tsTokens, null, 2),
      })
...
function simplifyTokens(
  ...
  if (isTransformedToken(tokens)) {
    const parsedToken = parseToken(
      tokenMap.get(tokens.$value as string) ?? tokens,
      tokenMap,
    )

    return parsedToken.css.value
  }
  ...
  return reduce(
    ...
    (acc, value, key) => {
      ...
      return { ...acc, [key]: simplifyTokens(value, tokenMap) }
    },
    {},
  )
}
...
function parseToken(token: TransformedToken, tokens?: DictionaryTokenMap) {
